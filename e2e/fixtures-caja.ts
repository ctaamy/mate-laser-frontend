...
export const CUENTAS = [
  { id: 'c-banco-tami', nombre: 'Banco Tami', tipo: 'banco', titular: 'Tami', alias: 'mlaser', es_del_negocio: true, saldo_inicial: 100000, fecha_inicio: '2026-09-01', orden: 0, archivada: false, saldo: 98500 },
  { id: 'c-mp', nombre: 'Mercado Pago (Facu)', tipo: 'mercadopago', titular: 'Facu', alias: null, es_del_negocio: true, saldo_inicial: 20000, fecha_inicio: '2026-09-01', orden: 1, archivada: false, saldo: 20000 },
  { id: 'c-banco-facu', nombre: 'Banco Facu', tipo: 'banco', titular: 'Facu', alias: null, es_del_negocio: true, saldo_inicial: 3000, fecha_inicio: '2026-09-01', orden: 2, archivada: false, saldo: 3000 },
  { id: 'c-efectivo', nombre: 'Efectivo', tipo: 'efectivo', titular: null, alias: null, es_del_negocio: true, saldo_inicial: 5000, fecha_inicio: '2026-09-01', orden: 3, archivada: false, saldo: 5000 },
  { id: 'c-bolsillo-facu', nombre: 'Bolsillo de Facu', tipo: 'bolsillo', titular: 'Facu', alias: null, es_del_negocio: false, saldo_inicial: 0, fecha_inicio: '2026-09-01', orden: 100, archivada: false, saldo: -12000 },
] as const;
...
export async function mockCaja(page: Page, opts: OpcionesCaja = {}): Promise<Capturas> {
  const cap: Capturas = { movimientos: [], correcciones: [], transferencias: [], arqueos: [], setups: [], anulaciones: [], listados: [] };
  let configurada = opts.configurada ?? true;
  let intentosPost = 0;
  const lista = opts.movimientos ?? [mov()];

  const saldos = () => {
    const cuentas = configurada ? CUENTAS : [];
  ...
  await page.route('**/api/v1/caja/**', async (route: Route) => {
    ...
    if (ruta === '/cuentas' && metodo === 'GET') return route.fulfill({ json: configurada ? CUENTAS.map((c) => { const { saldo, ...resto } = c; void saldo; return resto; }) : [] });
