...
//
// Tiene que ser el DOMINIO REAL (matelaserstudio.com.ar): la API solo lo
// permite por CORS y el Brick de MP solo renderiza en el dominio registrado
// para la public key — en mate-laser-frontend.fly.dev el Brick no monta.
//
// Uso local:  SMOKE_BASE_URL=https://www.matelaserstudio.com.ar npm run test:smoke

const baseURL = process.env.SMOKE_BASE_URL;
if (!baseURL) {
  throw new Error(
    'SMOKE_BASE_URL no seteada — es la URL del frontend deployado a chequear ' +
      '(ej. https://www.matelaserstudio.com.ar)',
  );
}
...
export default defineConfig({
  ...
  // Holgado: el spec espera hasta 60s a que monte el Brick + navegación.
  timeout: 120_000,
  use: {
    baseURL,
